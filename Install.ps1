...
try {
    ...
    Write-Host ' BackupCam installer' -ForegroundColor Green
    Write-Host '====================================' -ForegroundColor Green
    ...
    if ($Remove) {
        Write-Step 'Removing BackupCam'
        $modsDir = Join-Path $GameDir 'Mods'
        $dll = Join-Path $modsDir 'BackupCam.dll'
        $dataDir = Join-Path $modsDir 'BackupCam'
        $found = (Test-Path $dll) -or (Test-Path $dataDir)
        ...
        if ($found) {
            ...
            Write-Host '  BackupCam was not installed' -ForegroundColor Yellow
        }
        Write-Host ''
        Write-Host 'RedLoader and your other mods were left alone.'
        Write-Host ''
    ...
    Write-Step 'Downloading BackupCam'
    $modZip = Join-Path $env:TEMP 'BackupCam.zip'
    Get-ReleaseZip -Repo 'JustinOros/sotf-backupcam' -Pattern '^BackupCam\.zip$' -OutFile $modZip | Out-Null

    Write-Step 'Installing BackupCam'
    $modsDir = Join-Path $GameDir 'Mods'
    New-Item -ItemType Directory -Force -Path $modsDir | Out-Null
    Remove-Item (Join-Path $modsDir 'BackupCam.dll') -Force -ErrorAction SilentlyContinue
    Remove-Item (Join-Path $modsDir 'BackupCam') -Recurse -Force -ErrorAction SilentlyContinue
    Expand-Archive -Path $modZip -DestinationPath $modsDir -Force
    Remove-Item $modZip -Force -ErrorAction SilentlyContinue

    if (-not (Test-Path (Join-Path $modsDir 'BackupCam.dll'))) {
        throw 'Something went wrong, BackupCam.dll is not in the Mods folder'
    }
    if (-not (Test-Path (Join-Path $modsDir 'BackupCam\manifest.json'))) {
        throw 'Something went wrong, manifest.json is not in the Mods\BackupCam folder'
    }
    Write-Host '  installed' -ForegroundColor Green
    ...
    Write-Host '  1. Check that MODS appears on the main menu and lists BackupCam'
    Write-Host '  2. Load into a game and drive a golf cart'
    Write-Host '  3. Reverse to show the backup camera'
    Write-Host ''
