...
if ($Install) {
    ...
    $modDataDir = Join-Path $modsDir 'BackupCam'
...
if ($Package) {
    ...
    $zip = Join-Path $PSScriptRoot 'BackupCam.zip'

    if (Test-Path $stage) { Remove-Item $stage -Recurse -Force }
    New-Item -ItemType Directory -Force -Path (Join-Path $stage 'BackupCam') | Out-Null
    Copy-Item $dll -Destination $stage -Force
    Copy-Item (Join-Path $PSScriptRoot 'manifest.json') -Destination (Join-Path $stage 'BackupCam') -Force
